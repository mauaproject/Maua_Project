...
function scheduleLifecycleStatus(array $schedule, ?DateTimeImmutable $now = null): string
{
    ...
    if (!empty($schedule['archived_at']) || $endAt < $now) {
        return 'archived';
    }
...
function mapBookingRecord(
    ...
    $isArchived = !empty($booking['archived_at']) || $endAt < $now;
    $tripDriveLinkUrl = '';
...
function mapWorkerTask(array $task): array
{
    return [
        ...
        'scheduleDatabaseId' => nullableInt($task['booking_schedule_id'] ?? null),
        'sessionDatabaseId' => nullableInt($task['booking_session_id'] ?? null),
        'scheduleId' => $task['booking_schedule_code'] ?? '',
        'sessionId' => $task['booking_session_code'] ?? '',
        'sessionName' => $task['booking_schedule_name'] ?? $task['booking_session_name'] ?? '',
        'customerName' => $task['booking_customer_name'] ?? '',
        'customerEmail' => $task['booking_customer_email'] ?? '',
        'customerPhone' => $task['booking_customer_whatsapp'] ?? '',
        'whatsapp' => $task['booking_customer_whatsapp'] ?? '',
        'selectedDate' => $task['booking_selected_date'] ?? '',
        'requestedDate' => $task['booking_selected_date'] ?? '',
        'startTime' => !empty($task['booking_start_time']) ? substr((string) $task['booking_start_time'], 0, 5) : '',
        'endTime' => !empty($task['booking_end_time']) ? substr((string) $task['booking_end_time'], 0, 5) : '',
        'participants' => isset($task['booking_participants']) ? (int) $task['booking_participants'] : 0,
        'participantCount' => isset($task['booking_participants']) ? (int) $task['booking_participants'] : 0,
